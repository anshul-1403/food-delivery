                  ...
                  <TableCell>
                    <Stack direction="row" spacing={1} alignItems="center">
                      <Chip
                        ...
                        sx={{ fontWeight: "bold" }}
                      />
                      {order.cancellationRequest?.requestedBy === 'delivery' && !order.cancellationRequest.isProcessed && (
                        <Tooltip title={`Partner reported issue: ${order.cancellationRequest.reason}`}>
                          <Chip 
                  ...
                  <TableCell>
                    <Stack direction="row" spacing={1}>
                      <Select
                        ...
                        sx={{ color: "var(--text-color)", "& .MuiOutlinedInput-notchedOutline": { borderColor: "var(--border-color)" }, minWidth: 120 }}
                      >
                      ...
                      {order.cancellationRequest?.requestedBy === 'delivery' && (
                        <Button 
                          size="small" 
                          variant="outlined" 
                          color="error" 
                          ...
                          onClick={() => {
                            toast.info("Opening message thread with customer...");
           ...
           <Table>
            <TableHead sx={{ backgroundColor: "var(--secondary-bg)" }}>
              <TableRow>
                ...
                <TableCell sx={{ color: "var(--text-color)", fontWeight: "bold" }}>Vehicle Info</TableCell>
                <TableCell sx={{ color: "var(--text-color)", fontWeight: "bold" }}>Contact</TableCell>
                <TableCell sx={{ color: "var(--text-color)", fontWeight: "bold" }}>Status</TableCell>
            ...
            <TableBody>
              {partners.map((p) => (
                <TableRow key={p._id}>
                  <TableCell sx={{ color: "var(--text-color)" }}>{p.name}</TableCell>
                  <TableCell sx={{ color: "var(--text-color)" }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      <Typography variant="body2" sx={{ fontWeight: 'bold' }}>{p.vehicleId}</Typography>
                      <IconButton size="small" onClick={() => {
                        const newId = prompt("Enter New Vehicle ID (Manual Override):", p.vehicleId);
                        if (newId) handleVehicleAction(p._id, "override", newId);
                      }}>
                        <SendIcon sx={{ fontSize: 14, opacity: 0.5 }} />
                      </IconButton>
                    </div>
                    <Typography variant="caption" sx={{ opacity: 0.6 }}>{p.vehicleType}</Typography>
